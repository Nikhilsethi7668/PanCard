...
const ShowPanData = () => {
    ...
    const fetchPanEntries = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/api/pan-entries/${user.id}`);
    ...
    useEffect(() => {
        ...
        const fetchEmails = async () => {
            try {
                const response = await axios.get(`http://localhost:4000/data/${selectedPan}`, {
                    params: { page: currentPage, limit: 100, userId: user.id },
                });
                ...
                {selectedPan ? (
                    <>
                        ...
                        <div className="mt-6 flex justify-between items-center">
                            <button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 bg-blue-600 text-white rounded-md">Previous</button>
                            <span>Page {currentPage} of {totalPages}</span>
                            <button onClick={() => setCurrentPage((prev) => prev + 1)} disabled={currentPage >= totalPages} className="px-4 py-2 bg-blue-600 text-white rounded-md">Next</button>
                        </div>
                    </>
                ) : (
